...
def main():
    ...
    if args.n + args.n_queries > total:
        raise ValueError(
            f"Requested {args.n} insertions and {args.n_queries} queries, "
            f"but only {total} total points available."
        )

    # --- randomly sample disjoint sets for inserts and queries ---
    all_indices = np.arange(total)
    rng.shuffle(all_indices)
    insert_idx = all_indices[:args.n]
    query_idx = all_indices[args.n:args.n + args.n_queries]

    accessor = DatasetAccessor(args.files)

    insert_points = np.array([accessor[i] for i in insert_idx])
    query_points = np.array([accessor[i] for i in query_idx])

    recall_eps, cr_ann_acc, mem_mb = build_ann(
        accessor, insert_idx, args.r, args.epsilon, args.K, args.eta, query_points
    )

    print(f"[SUMMARY] r={args.r}, eps={args.epsilon}, eta={args.eta}, "
          f"recall={recall_eps:.4f}, (c,r)-ANN accuracy={cr_ann_acc}, memory={mem_mb}")
